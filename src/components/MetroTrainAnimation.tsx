...
import { metroTrains, depotLayout } from '../data/metroMockData';
...
const MetroTrainAnimation: React.FC<TrainAnimationProps> = ({ selectedTrain, onTrainSelect }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (Math.random() > 0.7) {
        const activeTrains = metroTrains.filter(t => t.status === 'Active');
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="relative bg-gray-50 rounded-lg p-4 overflow-hidden" style={{ height: '600px' }}>
        ...
        <svg width="100%" height="100%" viewBox="0 0 500 500" className="absolute inset-0 pointer-events-none">
          {animatingTrains.size > 0 && Array.from(animatingTrains).map((trainId) => {
            const train = metroTrains.find(t => t.id === trainId);
      ...
      {selectedTrain && (
        ...
        >
          {(() => {
            const train = metroTrains.find(t => t.id === selectedTrain);
            if (!train) return null;
...
export default MetroTrainAnimation;