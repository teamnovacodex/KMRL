...
import { historicalData, metroTrains } from '../data/metroMockData';
...
export const FleetStatusChart: React.FC<ChartProps> = ({ height = 300 }) => {
  const statusData = [
    { name: 'Service Ready', value: metroTrains.filter(t => t.recommendation === 'Service').length, color: '#10b981' },
    { name: 'Standby', value: metroTrains.filter(t => t.recommendation === 'Standby').length, color: '#f59e0b' },
    { name: 'Maintenance', value: metroTrains.filter(t => t.recommendation === 'Maintenance').length, color: '#ef4444' },
    { name: 'Deep Clean', value: metroTrains.filter(t => t.recommendation === 'Deep Clean').length, color: '#8b5cf6' }
  ];
...
export const HealthScoreDistribution: React.FC<ChartProps> = ({ height = 300 }) => {
  const healthData = metroTrains.map(train => ({
